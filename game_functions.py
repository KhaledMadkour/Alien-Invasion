...
def check_events( run , ai_settings, screen, stats, sb, play_button, 
                 ship, aliens,bullets):
    ...
    for event in pygame.event.get():
            if event.type == pygame.QUIT:               
                ...
                check_keydown_events(event, ai_settings, screen, ship, bullets,run)
...
def check_keydown_events(event, ai_settings, screen, ship, bullets, run):
...
def get_number_aliens_x(ai_settings, alien_width):
    ...
    number_aliens_x = int(avilable_space_x / (1.5 * alien_width))
...
def check_bullet_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    if len(aliens) == 0:
        # Destroy existing bullets and start new lvl.
        bullets.empty()
...
def check_aliens_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            #Treat this same as if sip got hit.
